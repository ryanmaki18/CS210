...
def centroid(points: list[tuple[int, int]]) -> tuple[int, int]:
    ...
    for i in points:
        ...
        x += a
        y += b
    mean_x = int(x / len(points))
    mean_y = int(y / len(points))
    mean_pair = mean_x, mean_y
    return mean_pair
...
def closest_index(point: tuple[int, int], centroids: list[tuple[int, int]]) -> int:
    ...
    """
    closest = 0
    distance = sq_dist(point, centroids[0])
    for i in range(len(centroids)):
        if sq_dist(point, centroids[i]) <= distance:
            distance = sq_dist(point, centroids[i])
            closest = i
    return closest
...
def assign_closest(points: list[tuple[int, int]],
    ...
    for i in range(len(centroids)):
        assignments.append([])
    for j in points:
        choice = closest_index(j, centroids)
